 ...
 */
export interface RouteNode<R, Child, C extends RouteNode<unknown, unknown, any>[] = []> {
 ...
 * @internal
 */
export type ChildUnion<C extends RouteNode<unknown, unknown, any>[]> = {
...
export function walkParse(
  nodes: RouteNode<unknown, unknown, RouteNode<unknown, unknown, any>[]>[],
  urlSegments: string[],
  ...
  for (const node of nodes) {
    ...
    if (rest.length > 0) {
      ...
      const child = walkParse(
        node.children as RouteNode<unknown, unknown, RouteNode<unknown, unknown, any>[]>[],
        rest,
        query,
        nextParams,
      );
 ...
 *               derive the full nested {@link InferRoute} union statically
 */
export function section<C extends RouteNode<unknown, unknown, any>[]>(
  path: string,
  children: [...C],
 ...
 */
export function defineRoutes<C extends RouteNode<unknown, unknown, any>[] = []>(children: [...C]) {
  ...
  const nodes = children as RouteNode<unknown, unknown, RouteNode<unknown, unknown, any>[]>[];
