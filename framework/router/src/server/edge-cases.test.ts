...
describe('Server over single-route router', () => {
  it('returns 404 when request path does not match the only registered route', async () => {
    const Only = object({ tag: literal('only') });
...
describe('Body and query types', () => {
  const GetRoute = object({ tag: literal('get-item'), id: string() });
  const PostRoute = object({ tag: literal('post-item') });
...
describe('Path segment edge cases in server', () => {
  const PairRoute = object({ tag: literal('pair'), x: string(), y: string() });
  ...
  it('wildcard route: server matches any URL pattern', async () => {
    const CatchAll = object({ tag: literal('catch-all'), rest: string() });
...
describe('HTTP method edge cases', () => {
  ...
  const GetRoute = object({ tag: literal('get-item'), id: string() });
  const DeleteRoute = object({ tag: literal('delete-item'), id: string() });
  const PutRoute = object({ tag: literal('put-item'), id: string() });
